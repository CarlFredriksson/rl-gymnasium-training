...
def plot_returns_multiple_runs(returns_per_run, target=None):
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            if i < len(returns_per_run):
                ...
                axs[row, col].plot(episodes, rolling_average(returns, 100), label="Rolling average return last 100 episodes")
                if target is not None:
    ...
    #fig.legend(handles, labels, loc="lower center", bbox_to_anchor=(0., 0., 0., 0.))
